...
class VentaneoClas(pg.GraphicsLayoutWidget):
    ...
    def __init__(self, parent,*args, **kwargs): 
        ...
        y=self.parent.ecg_Posiciones

        boton = QPushButton("Soy un boton")
        # boton.clicked.connect(self.metodoBoton)

        layout = QVBoxLayout() 

        print("self.parent.canal1 = ",self.parent.canal1)               

        if self.parent.canal1 != 0:     #canal1 es un bool definido en MainWindow

            print("arreglo1 en ventaneoR",self.arreglo1)               

            p1 = self.addPlot(title="subventana")            
            curvaP = p1.plot(pen='y')             
            curvaP.setData(y,self.arreglo1)
